<script setup>
    ...
    let route = useRoute()
    let store = useBebidasStore()

    let paginaInicio = computed(() => route.name === 'inicio')
        ...
        <div class="mx-auto container px-5 py-16">
            <div class="flex justify-between items-center">
                ...
                <nav class="flex gap-4 text-white ">
                    <RouterLink :to="{name: 'inicio'}" class="uppercase font-bold" active-class="text-orange-500">Inicio</RouterLink>

                    <RouterLink :to="{name: 'favoritos'}" class="uppercase font-bold" active-class="text-orange-500">Favoritos</RouterLink>
                </nav>
            </div>
            ...
            <form v-if="paginaInicio" class="md:w-1/2 2xl:w-1/3 bg-orange-400 my-32 p-10 rounded-lg shadow space-y-6" @submit.prevent="handleSubmit">
                ...
                <input type="submit" class="bg-orange-800 hover:bg-orange-900 cursor-pointer text-white font-extrabold w-full p-2 rounded-lg uppercase" value="Buscar Bebidas"/>
            </form>
        </div>
